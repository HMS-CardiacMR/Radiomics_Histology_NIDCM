...
for group in tqdm(list_of_groups[8:9]):
    list_of_sequences = os.listdir("Data_processed/"+group)
    for sequence in list_of_sequences[5:6]:
        # perform_clustering("Data_processed/" + group + "/" + sequence + "/Without_correlation/Without_PCA/")
        # perform_clustering("Data_processed/" + group + "/" + sequence + "/Without_correlation/PCA/")
        perform_clustering("Data_processed/" + group + "/" + sequence + "/With_correlation/Without_PCA/")
        # perform_clustering("Data_processed/" + group + "/" + sequence + "/With_correlation/PCA/")